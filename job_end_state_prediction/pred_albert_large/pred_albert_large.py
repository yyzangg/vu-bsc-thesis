...
class ALBERTWithNumericalFeatures(nn.Module):
    def __init__(self, albert_model, numerical_input_dim, numerical_hidden_dim, output_dim):
        ...
        self.fc2 = nn.Linear(numerical_hidden_dim, 1024)
        self.fc3 = nn.Linear(1024 + 1024, output_dim)
    ...
    def forward(self, numerical_input, input_ids, attention_mask):
        ...
        albert_pooled_output = albert_outputs.pooler_output
...
def train_model(model, train_dataloader, criterion, optimizer, scheduler, epochs):
    ...
    for epoch in range(epochs):
        ...
        for batch in tqdm(train_dataloader, desc=f'Epoch {epoch + 1}/{epochs}'):
            if len(batch[0]) == 1:  # Skip batches with a single sample
                continue
...
def run_multiple_iterations(X, y, n_iterations=3, test_size=0.2, epochs=4):
    metrics_list = []
    ...
    for i in range(n_iterations):
        ...
        albert_model = AlbertModel.from_pretrained('albert-large-v2')

        # Define hyperparameters
        model = ALBERTWithNumericalFeatures(albert_model, numerical_input_dim=4, numerical_hidden_dim=128, output_dim=7).to(device)
...
run_multiple_iterations(X, y, n_iterations=2, epochs=4)
