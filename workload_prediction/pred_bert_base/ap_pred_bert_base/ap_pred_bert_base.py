...
# Create text prompts based on findings
text_prompts = []
for _, row in df.iterrows():
    prompt = f"[CLS] Job submissions and job failures have a diurnal pattern (9 to 18). The job was submitted at {row['submit_hour_of_day']}."
    prompt += f" Failed jobs generally consume more CPUs. This job requested {row['NCPUS']} CPUs."
    text_prompts.append(prompt)

# Train-test split
X_train, X_test, y_train, y_test, text_train, text_test = train_test_split(X, y, text_prompts, test_size=0.2, random_state=0)
...
train_encodings = tokenizer(text_train, truncation=True, padding=True, max_length=128, return_tensors='pt')
test_encodings = tokenizer(text_test, truncation=True, padding=True, max_length=128, return_tensors='pt')
