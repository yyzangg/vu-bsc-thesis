...
from transformers import BertTokenizer, TFBertLMHeadModel, DataCollatorForLanguageModeling, create_optimizer
...
for epoch in range(num_epochs):
    ...
    for step, (input_ids, attention_mask, labels) in enumerate(train_dataset):
        # Adjust the shape of labels to match the shape of logits
...
def generate_text(prompt, model, tokenizer, max_new_tokens=50, num_return_sequences=1):
...
generated_table.to_csv('out_bert_large.csv', index=False)
