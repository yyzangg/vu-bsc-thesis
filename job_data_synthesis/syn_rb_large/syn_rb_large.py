...
# Step 3: Fine-Tuning
model = RobertaForCausalLM.from_pretrained('roberta-large')
...
def generate_text(prompt, model, tokenizer, max_length=256, num_return_sequences=1, temperature=1.5):
...
def generate_new_rows(input_table, model, tokenizer, scaler, num_new_rows, max_attempts=5):
    generated_rows = []
    generated_set = set()
    current_num_rows = 0
    ...
    while current_num_rows < num_new_rows:
        ...
        while attempts < max_attempts:
            ...
            generated_text = generate_text(prompt, model, tokenizer, num_return_sequences=1, temperature=1.5)[0]

            try:
                new_row = parse_generated_text(generated_text)
                if tuple(new_row) not in generated_set:
                    generated_set.add(tuple(new_row))
...
generated_table.to_csv('out_rb_large.csv', index=False)
